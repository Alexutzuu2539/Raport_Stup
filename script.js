...
async function fetchSheetData(sheetName = currentSheet) {
    return new Promise((resolve, reject) => {
        try {
            ...
            // Încercăm să folosim date de test dacă toate celelalte eșuează
            const useSampleData = () => {
                ...
                for (let i = 30; i >= 0; i--) {
                    ...
                    // Adăugăm și obiectul date pentru sortare
                    row.dateObj = date;
                ...
                .catch(error => {
                    ...
                    window[callbackName] = function(data) {
                        ...
                        if (!data || !data.feed || !data.feed.entry || data.feed.entry.length === 0) {
                            logError(`Date invalide primite pentru foaia ${sheetName}`);
                            
                            // Încercăm să folosim date de test ca ultimă soluție
                            const testData = useSampleData();
                            resolve(testData);
                            return;
                        }
                        ...
                        if (rows.length < 2) {  // cel puțin antete + un rând de date
                            logError(`Prea puține rânduri pentru foaia ${sheetName}: ${rows.length}`);
                            document.getElementById('table-body').innerHTML = 
                        ...
                        if (headerRow.length < 5) {
                            logError(`Antete insuficiente pentru foaia ${sheetName}: ${headerRow.join(', ')}`);
                            document.getElementById('table-body').innerHTML = 
                        ...
                        // Stocăm datele și rezolvăm promisiunea
                        sheetsData[sheetName] = rows;
                        resolve(rows);
                    };
                    
                    // Creăm scriptul pentru a încărca datele prin JSONP
                    script = document.createElement('script');
                    script.id = scriptId;
                    // URL-ul pentru API-ul JSON cu callback-ul definit mai sus
                    const jsonUrl = `https://spreadsheets.google.com/feeds/cells/${SHEET_IDS[sheetName]}/1/public/values?alt=json-in-script&callback=${callbackName}&timestamp=${Date.now()}`;
                    script.src = jsonUrl;
                    
                    // Gestionăm erorile de încărcare a scriptului
                    script.onerror = function() {
                        logError(`Eroare la încărcarea scriptului JSONP pentru foaia ${sheetName}`);
                        delete window[callbackName];
                    ...
                    // Adăugăm scriptul în pagină
                    document.head.appendChild(script);
                    
                    // Setăm un timeout pentru a gestiona cazul în care scriptul nu se încarcă
                    setTimeout(function() {
                        if (window[callbackName]) {
                            logError(`Timeout la încărcarea datelor JSONP pentru foaia ${sheetName}`);
                            delete window[callbackName];
...
async function initPage() {
    ...
    // Adăugăm listener pentru butonul de debugging
    if (document.getElementById('debug-btn')) {
        document.getElementById('debug-btn').addEventListener('click', testConnections);
    }
    ...
    if (!allUrlsOk) {
        logError("Unele URL-uri nu sunt accesibile. Verificați conexiunea și URL-urile.");
        document.getElementById('error-container').style.display = 'block';
    }
    ...
    try {
        ...
        if (Object.keys(sheetsData).length === 0) {
            logError("Nu s-au putut prelua datele pentru niciun stup");
            document.getElementById('table-body').innerHTML = 
        ...
        if (combinedData && combinedData.length > 0) {
            ...
            logError("Nu există date combinate");
            document.getElementById('table-body').innerHTML = 
        ...
        logError('Eroare la inițializare', error);
        document.getElementById('table-body').innerHTML = 
